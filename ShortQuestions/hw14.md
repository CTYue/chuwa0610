...
9. Spring WebFlux vs Stream API+CompletableFuture, why use Spring WebFlux over the other? 
   ...
   >3. **Better Resource Efficiency**: WebFlux’s non-blocking nature allows it to handle more requests with fewer threads, reducing the need for hardware resources as compared to traditional servlet-based models, which can be more resource-intensive when scaling up.
