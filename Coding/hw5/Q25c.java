...
public class Q25c {
    ...
    public static void main(String[] args) {
        try {
            CompletableFuture<String> productsFuture = fetchData(PRODUCTS_API,"Default");
            CompletableFuture<String> reviewsFuture = fetchData(REVIEWS_API,"Default");
            CompletableFuture<String> inventoryFuture = fetchData(INVENTORY_API,"Default");
    ...
    private static CompletableFuture<String> fetchData(String apiUrl, String defaultValue) {
        HttpRequest request = HttpRequest.newBuilder(URI.create(apiUrl)).GET().build();

        return client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(HttpResponse::body).exceptionally((e)->{return defaultValue;});
    }
}
