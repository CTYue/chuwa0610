...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        Optional<String> res = Optional.ofNullable(customer).map(c ->c.getShoppingCart()).map(sc->sc.getItems()).filter(i-> !i.isEmpty()).map(i -> i.get(0)).map(i->i.getName());
        return res;
    }
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        double res = customer.map(c -> c.getShoppingCart()).map(cs->cs.getItems()).filter(i-> !i.isEmpty()).map(i -> i.stream().mapToDouble(item -> item.getPrice()).sum()).orElse(0.0);
        return res;
    }
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        String res = Optional.ofNullable(customer).map(c->c.getShoppingCart()).map(cs -> cs.getItems()).filter(i -> !i.isEmpty()).map(i -> i.get(0)).map(i->i.getName()).orElseGet(ShoppingCartUtil::getRandomAlternativeItem);
        return res;
    }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        String res = Optional.ofNullable(customer).map(c -> c.getShoppingCart()).map(cs ->cs.getItems()).filter(i -> !i.isEmpty()).map(i -> i.get(0)).map(i->i.getName()).orElseThrow(() ->new EmptyCartException("empty cart"));
        return res ;
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        Optional<List<Item>> loI = Optional.ofNullable(customer).map(c -> c.getShoppingCart()).map(cs ->cs.getItems());
        if(loI.isPresent() && loI.get().size() > 1) {
    ...
    public static void printItemsInCart(Customer customer) {
        ...
        if(loI.isPresent() && loI.get().size() > 1) {
            loI.get().forEach(item -> System.out.println(item.getName()));
        }
    }
